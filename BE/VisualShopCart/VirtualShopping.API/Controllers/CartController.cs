...
namespace VirtualShopping.Controllers
{
    public class CartController : BaseController
    {
        ...
        [HttpGet]
        [Route("cart-by-customer")]
        public async Task<IActionResult> GetCartByCustomer(string customerId)
        {
            if (string.IsNullOrEmpty(customerId))
            {
                return BadRequest();
            }
            var cartInfo = await _cartServices.GetCartByCustomerId(customerId).ConfigureAwait(false);
